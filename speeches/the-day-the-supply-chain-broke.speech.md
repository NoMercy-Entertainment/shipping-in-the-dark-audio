...
<!-- p-10 -->
Bug one: the error body was thrown away. The GenericHttpClient, which handles all outbound HTTP requests from the media server, had a catch block that logged the exception message but never read the response body. Keycloak was sending back a perfectly descriptive error explaining exactly what went wrong. The catch block swallowed it whole. When you're debugging a 400 error and the logs say "Response status code does not indicate success: 400 Bad Request" with zero additional context, you are having a bad time. That Keycloak error body was the entire diagnosis, and the code threw it in the garbage.
...
<!-- p-52 -->
The scanner checked for the disguised PowerShell executable, the Python RAT, active network connections to the attacker's server, the plain-crypto-js package in any node modules directory, running processes with names matching the malware's disguises, and the poisoned axios versions in any lockfile on the system.
